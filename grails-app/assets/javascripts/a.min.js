$(document).ready(function(){$(".recurringAppointmentAdminOptions").fadeOut();});function getTimeSlotOptions(){var a=$("#services").val();var c=$("#dateOfAppointment").val();
...
$.ajax({type:"POST",url:a+"/getSection",data:{section:b}}).done(function(d){$(".active").removeClass("active");$('a[href="'+b+'"]').parent().addClass("active");
$(".main").html(d);$(".navbar-toggle").click();});});$(document).on("tap","#addClient",function(b){var a=$("body").attr("baseUrl");$.ajax({type:"POST",url:a+"/getClientDataForm"}).done(function(c){if(c.indexOf("ERROR")>-1){alert("Dang... you broke it.");
...
}});});$(document).on("tap","#editClient",function(c){var a=$("#clientsDetailsSelector").val()[0];$("#clientsDetailsSelector").val()[0];var b=$("body").attr("baseUrl");
...
$("#clientsDetailsSelector").slideUp();$("#clientDetails").slideUp();$("#addClient").fadeOut();$("#clientInfoForm").html(d).slideDown();}});});$(document).on("tap",".last-name-filter",function(b){var a=$(b.currentTarget).attr("value");
...
$("#clientsDetailsSelector").children("optgroup").fadeOut();$("#clientsDetailsSelector").children("optgroup[label='"+a+"']").fadeIn();}});$(document).on("tap","#saveClientButton",function(c){var d=$("#e").val();
...
if(k===-1){hideClientRegistrationForm();alert("User saved!");}});}});$(document).on("tap","#cancelClientRegistrationButton",function(a){hideClientRegistrationForm();
...
$("#addClient").fadeIn();}$(document).on("tap",".cancel-appointment-button",function(b){var d=$(this).attr("c");var a=$("body").attr("baseUrl");$.ajax({type:"POST",url:a+"/cancelAppointment",data:{c:d}}).done(function(c){var e=c.search('"success":false');
