$(document).ready(function(){getSection("upcomingAppointments");});function getTimeSlotOptions(){var a=$("#services").val();var c=$("#dateOfAppointment").val();
...
});$(document).on("tap",".appointment-data",function(a){var c=a.currentTarget;if(!$(c).hasClass("selected")){hideAppointmentData();$(c).addClass("selected");
var b=c.id;$(".edit-appointment-"+b).fadeIn();getRescheduleOptions(b);}else{hideAppointmentData();}});function hideAppointmentData(){$(".edit-appointment").fadeOut();
$(".appointment-data.selected").removeClass("selected");}function getRescheduleOptions(a){var b=$("body").attr("baseUrl");$.ajax({type:"POST",url:b+"/getRescheduleOptions",data:{aId:a}}).done(function(c){if(c.indexOf("ERROR")===-1){$("#edit-appointment-options").html(c);
