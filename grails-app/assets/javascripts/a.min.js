...
if(a){$(".navbar-toggle").click();}});$(document).on("tap",".nav a",function(c){var b=$(this).attr("href");var a=$("body").attr("baseUrl");if(b==="#logout"){window.location.href="./access/logout";
}else{$("#mask").fadeIn();$.ajax({type:"POST",url:a+"/getSection",data:{section:b}}).done(function(d){$(".active").removeClass("active");$('a[href="'+b+'"]').parent().addClass("active");
...
var a=$(this).val();var b=$("body").attr("baseUrl");$.ajax({type:"POST",url:b+"/getClientDetails",data:{cId:a}}).done(function(c){var d=c.search('"success":false');
...
if(i.success===true){$("#rescheduleButton-"+c).html("Success");$("#rescheduleButton-"+c).removeClass("error-button");setTimeout(function(){window.location.href="./";
},1250);}else{$("#rescheduleButton-"+c).html("Error");$("#rescheduleButton-"+c).addClass("error-button");}});});}});$(".edit-appointment-"+c).slideDown();
});